...
Usage:
  ...
  Attachments:
    asana-cli attachments <id>                 List attachments on task
    asana-cli download <attachment_id> [--output path]  Download attachment
    asana-cli upload <id> <file_path>          Upload file as attachment
...
def _run_command(cmd, args, token, config):
    ...
    if cmd in ("list", "ls"):
        ...
        if len(args) < 2:
            print("Usage: asana-cli attachments <task_id>", file=sys.stderr)
            sys.exit(1)
        cmd_attachments(token, args[1])
    elif cmd == "download":
        if len(args) < 2:
            print("Usage: asana-cli download <attachment_id> [--output <path>]", file=sys.stderr)
            sys.exit(1)
        output = None
        if "--output" in args:
        ...
        if len(args) < 3:
            print("Usage: asana-cli upload <task_id> <file_path>", file=sys.stderr)
            sys.exit(1)
